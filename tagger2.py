...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        self.txt_box = gr.Textbox(label='選中的tags會自動追加到提示詞裡', interactive=True)  # adds text box
        self.txt_box.change(
            fn=self.textbox_changed,
            inputs=self.txt_box,
            outputs=self.txt_box
        )
        #self.weighting_chkbx = gr.Checkbox(label='增加注意括弧',value=False)  # adds the attention checkbox
        ...
        if os.path.exists(folder_dir):  # now we pull out all the tags in the files
            with gr.Tabs(elem_id="tags_tabs") as self.tabs:               
                ...
                if '最愛.txt' in txt_files_list:
                    index = txt_files_list.index('最愛.txt')
